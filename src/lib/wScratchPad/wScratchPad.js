      ...
      if (this.options.fg) {
        if (this.options.fg.charAt(0) === '#') {
          ...
          .load(function () {
            ...
            var cropHeight = Math.floor(cropWidth/scale);

            // make sure the crop height is less than or equal to the image's original height
            // or the drawImage action will be failed. for example at iphone6 plus
            cropHeight = cropHeight > this.height ? this.height : cropHeight;

            _this.ctx.drawImage(this, 0, 0, cropWidth, cropHeight, 0, 0, width, height);
            _this.$img.show();
          });
        }
      }
